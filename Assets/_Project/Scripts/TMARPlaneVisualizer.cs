...
public class TMARPlaneVisualizer : MonoBehaviour
{
    ...
    Color largestColor = Color.magenta;

    Color farthestColor = Color.red;
    ...
    void Update()
    {
        ...
        // If this plane is the largest, indicate that with the largestColor value
        if (m_ARPlane == FindFirstObjectByType<TMARSceneController>().LargestPlane)
        {
            m_MeshRenderer.material.color = largestColor;
        }

        // If this plane is the farthest, indicate that with the farthestColor value
        if (m_ARPlane == FindFirstObjectByType<TMARSceneController>().FarthestPlane)
        {
            m_MeshRenderer.material.color = farthestColor;
        }
    }

    void UpdateDebugInfo()
    {
        m_DebugLabelOffset.position = m_ARPlane.center + Vector3.up * 0.5f;
